...
# Every builder takes a parsed Statement, never a string, so each of these
# parses first. That is the interface: the tree is made once, at the door, and
# no builder gets a second opinion about SQL that will not parse.

def _group(sql):
    return build_group_probe(Statement.parse(sql))


def _probe(sql):
    return build_probe(Statement.parse(sql))


def _taint(sql, columns):
    return build_taint_probe(Statement.parse(sql), columns)


def _update(sql, key_columns=None):
    return build_rollback(Statement.parse(sql), "AIRLOCK.SNAP_X",
                          key_columns=key_columns)


def _snap(sql, table="AIRLOCK.SNAP_T"):
    return snapshot_sql(Statement.parse(sql), table)
...
def test_group_probe_uses_an_identifier_exasol_accepts():
    ...
    probe = _group("SELECT AVG(C_ACCTBAL) FROM TPCH.CUSTOMER")
    assert "_G " not in probe and "(_" not in probe


def test_group_probe_ignores_a_write():
    assert _group("UPDATE TPCH.CUSTOMER SET C_COMMENT = 'x'") is None


def test_group_probe_survives_unparseable_sql():
    assert _group("SELECT FROM WHERE ;;") is None


def test_blast_radius_probe_counts_what_the_write_would_touch():
    probe = _probe("UPDATE TPCH.CUSTOMER SET C_COMMENT = 'x' WHERE C_ACCTBAL > 0")
...
def test_taint_probe_takes_the_worst_of_several_columns():
    probe = _taint("SELECT * FROM TPCH.SUPPLIER", ["S_ADDRESS", "S_COMMENT"])
...
def test_taint_probe_does_not_wrap_a_single_column_in_greatest():
    probe = _taint("SELECT C_COMMENT FROM TPCH.CUSTOMER", ["C_COMMENT"])
...
def test_taint_probe_is_skipped_when_no_text_comes_back():
    assert _taint("SELECT COUNT(*) FROM TPCH.CUSTOMER", []) is None


def test_taint_probe_ignores_a_write():
    assert _taint("UPDATE TPCH.CUSTOMER SET C_COMMENT = 'x'", ["C_COMMENT"]) is None
...
def test_rollback_reinserts_what_a_delete_removed():
    ...
    rollback = build_rollback(Statement.parse(sql), "AIRLOCK.SNAP_X", ["O_ORDERKEY"])
...
def test_rollback_carries_no_placeholders_for_any_write_kind():
    ...
    for sql in ("UPDATE TPCH.ORDERS SET O_ORDERPRIORITY = '1-URGENT' WHERE O_ORDERKEY > 1",
        ...
        rollback = build_rollback(Statement.parse(sql), "AIRLOCK.SNAP_X",
                                  ["O_ORDERKEY"])
...
# ---------------------------------------------------------------------------
# one parse, one answer
# ---------------------------------------------------------------------------

UNPARSEABLE = "UPDATE TPCH.CUSTOMER SET WHERE WHERE"


def test_an_unparseable_statement_has_no_tree_and_denies_by_kind():
    """The single place the question is answered. Everything below depends on
    this: `policy.evaluate` denies kind=OTHER, so no probe is ever reached."""
    stmt = Statement.parse(UNPARSEABLE)
    assert stmt.tree is None
    assert stmt.kind == "OTHER"
    assert stmt.parse_error


@pytest.mark.parametrize("build", [
    lambda s: build_probe(s),
    lambda s: build_group_probe(s),
    lambda s: build_taint_probe(s, ["C_COMMENT"]),
    lambda s: build_rollback(s, "AIRLOCK.SNAP_X", ["C_CUSTKEY"]),
    lambda s: snapshot_sql(s, "AIRLOCK.SNAP_X"),
], ids=["probe", "group", "taint", "rollback", "snapshot"])
def test_no_builder_invents_sql_for_a_statement_that_did_not_parse(build):
    """Each of these used to parse for itself and decide alone what a failure
    meant -- run unmeasured, hold, or deny as a bug. They now agree, because
    there is nothing left for them to disagree about."""
    assert build(Statement.parse(UNPARSEABLE)) is None
